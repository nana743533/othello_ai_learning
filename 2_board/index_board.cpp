...
// インデックスごとのマスの移動数
const int move_offset[n_board_idx] = {1, 1, 1, 1, 1, 1, 1, 1, 8, 8, 8, 8, 8,
...
int move_arr[2][n_line][hw][2]; // move_arr[プレイヤー][ボードのインデックス][マスの位置][0:
                                // 左 1: 右] = 何マスひっくり返せるか
bool legal_arr[2][n_line][hw]; // legal_arr[プレイヤー][ボードのインデックス][マスの位置]
                               // = trueなら合法、falseなら非合法
int flip_arr
    [2][n_line]
    [hw]; // flip_arr[プレイヤー][ボードのインデックス][マスの位置] =
          // ボードのインデックスのマスの位置をひっくり返した後のインデックス
int put_arr[2][n_line]
           [hw]; // put_arr[プレイヤー][ボードのインデックス][マスの位置] =
                 // ボードのインデックスのマスの位置に着手した後のインデックス
int local_place[n_board_idx]
               [hw2]; // local_place[インデックス番号][マスの位置] =
                      // そのインデックス番号におけるマスのローカルな位置
int place_included
    [hw2]
    [4]; // place_included[マスの位置] =
         // そのマスが関わるインデックス番号の配列(3つのインデックスにしか関わらない場合は最後の要素に-1が入る)
int reverse_board[n_line]; // reverse_board[ボードのインデックス] =
                           // そのインデックスにおけるボードの前後反転
int pow3[11]; // pow3[i] = 3^i
int pop_digit[n_line]
             [hw]; // pop_digit[ボードのインデックス][i] =
                   // そのインデックスの左からi番目の値(3進数なので0か1か2)
int pop_mid[n_line][hw]
           [hw]; // pop_mid[ボードのインデックス][i][j] =
                 // そのインデックスの左からi番目、左からj番目に挟まれる場所の値
...
public:
  ...
  bool operator<(const board &another) const { return value > another.value; }
  ...
  inline bool legal(int g_place) {
    ...
    for (int i = 0; i < 3; ++i)
      res |=
          legal_arr[this->player][this->board_idx[place_included[g_place][i]]]
                   [local_place[place_included[g_place][i]][g_place]];
    if (place_included[g_place][3] != -1)
      res |=
          legal_arr[this->player][this->board_idx[place_included[g_place][3]]]
    ...
    for (i = 0; i < hw2; ++i) {
      for (j = 0; j < 4; ++j) {
        if (place_included[i][j] == -1)
          continue;
        if (arr[i] == black)
          this->board_idx[place_included[i][j]] -=
              2 * pow3[hw - 1 - local_place[place_included[i][j]][i]];
        else if (arr[i] == white)
          this->board_idx[place_included[i][j]] -=
              pow3[hw - 1 - local_place[place_included[i][j]][i]];
        else if (j == 0)
          --this->n_stones;
      }
    }
